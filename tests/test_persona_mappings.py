...
from generate_posts import ACCOUNT_DATA, sample_account, sample_modifiers
...
class TestPersonaModifierMappings:
    ...
    def test_teen_personas_get_teenager_life_stage(self):
        ...
        for persona in teen_personas:
            ...
            assert modifiers["life_stage"] == "teenager", (
                f"{persona} should have teenager life stage"
            )
            ...
            if "anxiety_ridden" in persona:
                assert modifiers["education_level"] in [
                    "high_school_dropout",
                    "high_school_grad",
                ], f"{persona} should have high school education"
            else:
                assert modifiers["education_level"] in [
                    "high_school_grad",
                    "some_college",
                ], f"{persona} should have high school or some college education"
    ...
    def test_student_personas_get_college_life_stage(self):
        ...
        for persona in student_personas:
            ...
            assert modifiers["life_stage"] == "college_student", (
                f"{persona} should have college_student life stage"
            )
            assert modifiers["education_level"] in ["some_college", "college_degree"], (
                f"{persona} should have college-level education"
            )
    ...
    def test_professional_personas_get_young_professional_life_stage(self):
        ...
        for persona in professional_personas:
            ...
            assert modifiers["life_stage"] == "young_professional", (
                f"{persona} should have young_professional life stage"
            )
            ...
            if "lawyer" in persona:
                assert modifiers["education_level"] == "graduate_degree", (
                    f"{persona} should have graduate degree"
                )
            elif "teacher" in persona:
                assert modifiers["education_level"] in [
                    "college_degree",
                    "graduate_degree",
                ], f"{persona} should have college+ education"
            elif "nurse" in persona:
                assert modifiers["education_level"] in [
                    "college_degree",
                    "trade_school",
                    "graduate_degree",
                ], f"{persona} should have nursing-appropriate education"
            else:
                ...
                assert modifiers["education_level"] in valid_education, (
                    f"{persona} should have professional-level education"
                )
    ...
    def test_parent_personas_get_parent_life_stage(self):
        ...
        for persona in parent_personas:
            ...
            assert modifiers["life_stage"] == "parent", (
                f"{persona} should have parent life stage"
            )
            assert modifiers["primary_topic"] == "family_kids", (
                f"{persona} should focus on family topics"
            )
    ...
    def test_middle_aged_personas_get_middle_aged_life_stage(self):
        ...
        for persona in middle_aged_personas:
            ...
            assert modifiers["life_stage"] == "middle_aged", (
                f"{persona} should have middle_aged life stage"
            )

            # Special case: conspiracy theory uncle should have right-leaning politics
            if "conspiracy" in persona:
                assert modifiers["political_leaning"] in [
                    "far_right_extremist",
                    "conservative_republican",
                ], f"{persona} should have right-leaning politics"
    ...
    def test_retired_personas_get_retired_life_stage(self):
        ...
        for persona in retired_personas:
            ...
            assert modifiers["life_stage"] == "retired", (
                f"{persona} should have retired life stage"
            )
            ...
            if "boomer" in persona:
                assert modifiers["political_leaning"] in [
                    "conservative_republican",
                    "far_right_extremist",
                ], f"{persona} should have conservative politics"
    ...
    def test_anxiety_persona_gets_anxious_communication(self):
        ...
        assert modifiers["communication_style"] == "anxious_oversharing", (
            "Anxiety persona should have anxious communication"
        )
    ...
    def test_athlete_persona_gets_achievement_topics(self):
        ...
        assert modifiers["primary_topic"] == "achievements_bragging", (
            "Athlete persona should focus on achievements"
        )
...
class TestAccountTypeModifiers:
    ...
    def test_individual_account_modifiers(self):
        ...
        assert set(modifiers.keys()) == expected_categories, (
            "Individual accounts should have correct modifier categories"
        )
    ...
    def test_brand_business_account_modifiers(self):
        ...
        assert set(modifiers.keys()) == expected_categories, (
            "Business accounts should have correct modifier categories"
        )
    ...
    def test_influencer_account_modifiers(self):
        ...
        assert set(modifiers.keys()) == expected_categories, (
            "Influencer accounts should have correct modifier categories"
        )
    ...
    def test_media_news_account_modifiers(self):
        ...
        assert set(modifiers.keys()) == expected_categories, (
            "Media accounts should have correct modifier categories"
        )

    def test_bot_account_modifiers(self):
        """Bot accounts should only get bot-specific modifiers."""
        expected_categories = {"bot_personality", "response_pattern", "content_type"}

        modifiers = sample_modifiers("bot", "some_bot")
        assert set(modifiers.keys()) == expected_categories, (
            "Bot accounts should have correct modifier categories"
        )
    ...
    def test_spam_scam_account_modifiers(self):
        ...
        assert set(modifiers.keys()) == expected_categories, (
            "Spam accounts should have correct modifier categories"
        )
    ...
    def test_meme_account_modifiers(self):
        ...
        assert set(modifiers.keys()) == expected_categories, (
            "Meme accounts should have correct modifier categories"
        )
...
class TestAccountGeneration:
    ...
    def test_sample_account_returns_valid_structure(self):
        ...
        assert account["persona"] in ACCOUNT_DATA[account["account_type"]]["personas"]

        # Check modifiers match account type
        expected_modifiers = set(
            ACCOUNT_DATA[account["account_type"]]["modifiers"].keys()
        )
    ...
    def test_multiple_account_generation_consistency(self):
        ...
        for _ in range(50):  # Test with many samples
            ...
            if account["account_type"] == "individual":
                ...
                if any(
                    word in persona
                    for word in ["high_schooler", "teenage", "theatre_kid"]
                ):
                    assert modifiers["life_stage"] == "teenager"
                    ...
                    word in persona
                    for word in ["lawyer", "teacher", "designer", "tech_bro", "nurse"]
                ):
                    assert modifiers["life_stage"] == "young_professional"
                elif (
                    any(word in persona for word in ["mom", "dad", "parent"])
                    and "wine_mom" not in persona
                ):
                    assert modifiers["life_stage"] == "parent"
                    assert modifiers["primary_topic"] == "family_kids"
                elif any(
...
class TestSystemPromptGeneration:
    ...
    def test_system_prompt_consistent_sections(self):
        """System prompt should have consistent sections across account types."""
        from generate_posts import generate_system_prompt
        ...
        assert "CHARACTERISTICS:" in individual_prompt
        assert "Communication Style: Casual Friendly" in individual_prompt
        ...
        assert "CHARACTERISTICS:" in business_prompt
        assert "Brand Voice: Professional Corporate" in business_prompt
