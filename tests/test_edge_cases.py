...
class TestEdgeCases:
    ...
    def test_sample_modifiers_with_none_persona(self):
        ...
        assert len(modifiers) > 0, "Should still return modifiers even with None persona"
    ...
    def test_sample_modifiers_with_empty_persona(self):
        ...
        assert len(modifiers) > 0, "Should still return modifiers even with empty persona"
    ...
    def test_system_prompt_with_missing_modifiers(self):
        ...
        assert "test persona" in prompt.lower(), "Should still generate prompt with partial modifiers"
        assert "Communication: Casual Friendly" in prompt, "Should include available modifiers"
    ...
    def test_system_prompt_with_empty_modifiers(self):
        ...
        prompt = generate_system_prompt(account)
        assert "test persona" in prompt.lower(), "Should still generate prompt with no modifiers"
        assert "No specific modifiers defined" in prompt, "Should indicate missing modifiers"
...
class TestRandomness:
    ...
    def test_persona_sampling_within_type_shows_variation(self):
        ...
        assert len(unique_personas) > 1, "Should generate different personas within account type"
...
class TestBoundaryConditions:
    ...
    def test_very_long_persona_name(self):
        ...
        long_persona = "very_long_persona_name_that_goes_on_and_on_with_many_words_" * 10
    ...
    def test_case_sensitivity_in_persona_matching(self):
        ...
        assert lower_modifiers["life_stage"] == "teenager", "Lowercase should match pattern"
...
class TestConsistencyChecks:
    ...
    def test_same_persona_gets_consistent_deterministic_mappings(self):
        ...
        assert all(stage == "teenager" for stage in life_stages), "Life stage should be deterministic for anxiety_ridden_high_schooler"
        
        # Communication style should be deterministic for this specific persona
        assert all(style == "anxious_oversharing" for style in communication_styles), "Communication should be deterministic for anxiety_ridden_high_schooler"
        
        # Education should be from the restricted set
        valid_education = {"high_school_dropout", "high_school_grad"}
        assert all(edu in valid_education for edu in education_levels), "Education should be from valid teen set"
    ...
    def test_parent_personas_always_get_family_topics(self):
        ...
        for persona in parent_personas:
            for _ in range(5):  # Test multiple times
                modifiers = sample_modifiers("individual", persona)
                assert modifiers["life_stage"] == "parent", f"{persona} should always be parent"
                assert modifiers["primary_topic"] == "family_kids", f"{persona} should always focus on family"
    ...
    def test_professor_always_gets_graduate_degree(self):
        ...
        for _ in range(10):
            ...
            assert modifiers["education_level"] == "graduate_degree", "Professor should have graduate degree"